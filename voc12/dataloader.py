...
cls_labels_dict = np.load('/home/maruf/ws2m2/voc12/cls_labels.npy', allow_pickle=True).item()
...
def load_img_name_list(dataset_path):
    ...
    # following code is for the numpy version compatibality
    # numpy.version.version is 1.23.3
    img_name_list = np.loadtxt(dataset_path, dtype=str)
    img_name_list = np.array([np.int32(img_name.replace('_', '')) for img_name in img_name_list])
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, voc12_root,crop_size=None,
    ...
    def __getitem__(self, idx):
        ...
        return {'name': name, 'img': img, 'label': label, "size": (img.shape[0], img.shape[1]), "label_cls": torch.from_numpy(self.label_list[idx])}
