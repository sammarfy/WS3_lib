...
def run(args):
    ...
    for ep in range(args.cam_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            one_hot_vectors = torch.zeros(number_classes, label.shape[0])
            target = torch.where(label == 1)[0]
            ...
            for i, label in enumerate(target):
                ...
                # weights = torch.zeros_like(target_val).type(dtype)
                # weights[target_val>0] = 1.
            ...
            loss = F.multilabel_soft_margin_loss(out, label)
